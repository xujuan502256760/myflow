(function(b) {
    ...
    a.config = {
        ...
        path: {
            ...
            props: {
                ...
                //     }
                // }
                nameCh: {name: "nameCh",label: "中文名称", value: "", editor: function() { return new a.editors.inputEditor()}},
                condition: {name: "condition",label: "条件设置", value: "", editor: function() { return new a.editors.buttonEditor('设置')}},
                priority: {name: "priority",label: "优先级", value: "", editor: function() { return new a.editors.inputEditor()}},
                isDefault: {name: "isDefault",label: "是否默认", value: "", editor: function() { return new a.editors.selectEditor([{name:"是",value:"1"},{name:"否",value:"0"}])}},
                descr: {name: "descr",label: "描述", value: "", editor: function() { return new a.editors.inputEditor()}},
    ...
    a.rect = function(p, m) {
        ...
        b([t.node, f.node, n.node, e.node]).bind("click",//规则框点击事件
        function() {
            if (!a.config.editable) {
                return
            }
            ...
            b(C).trigger("click", u);//触发点击事件
            b(C).data("currNode", u);
            $we.app.flows.currSelectRule=b(C).data("currNode");//当前选中的对象
            return false
        });
        var j = function(o, r) {
            if (!a.config.editable) {
        ...
        function B( isOnload) {//实现每一个框（对应myflow.jpdl4.js 里面的myflow.config.tools.states ）
            var F = null,r=null;
            if( isOnload && isOnload === 'onload'){
                F = (q.x + E.margin) - $(".ui-droppable").offset().left;
                r = (q.y + E.margin) - $(".ui-droppable").offset().top;
            }else{
                F = q.x + E.margin;
                r = q.y + E.margin;
            }
    ...
    a.path = function(q, n, u, e) {
        ...
        m();
        b([i.node, t.node]).bind("click",//绑定点击事件
        function() {
            if (!a.config.editable) {
                return
            }
    ...
    a.props = function(h, f) {//属性栏
        ...
        b(g).bind("showprops", d)//绑定任务到点击事件
    };
    a.editors = {
        textEditor: function() {
            var d, e, c, g, f;
    ...
    a.init = function(x, r) {
        var v = b(window).width('100%'),//界面宽度
        e = b(window).height(),//界面高度
        y = Raphael(x, v * 1.5, e * 1.5),//画布
        q = {},//存储数据states
        g = {};//存储数据path
        b.extend(true, a.config, r);
       b(document).keydown(function (i,byButton) {//事件
            if (!a.config.editable) {
                return
            }
        ...
        b(document).click(function() {//线条和，任务点击事件
        ...
        // //属性里面按钮点击事件
        // b(document).bind("showChooseRuleView",showView)
        ...
        if (a.config.editable) {
            ...
            $we.app.flows.delFlow=function(){
                y.clear();//清除画布
                q={};//清除
                g={};
            }
            $we.app.flows.saveFlow=function(){
            
                $we.app.flows.data=flo();
            }
            //保存值
            var flo=function(){
                var i = "{states:{";
        ...
        if (r.restore) {//界面数据
            ...
            if (B.states) {
                for (var s in B.states) {
                    ...
                    d.restore(B.states[s]);//遍历states里面的每一个值 d.restore(B.states[s]=rect1);
                    z[s] = d;
                    q[d.getId()] = d
                }
            }
